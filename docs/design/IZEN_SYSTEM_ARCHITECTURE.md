...
>
> All file references are relative to the repository root and correspond to real files. Generated by static analysis of 671 Go files.
...
The system is split into **two parallel engine generations** that coexist today:
...
| **v1 Runtime (CLI `izen run`)** | `pkg/runtime/engine` + `pkg/runtime/wire` | State-driven orchestration `Receive → Analyze → Plan → EvaluatePolicy → Execute → Validate → Done/Recover` | `pkg/runtime/engine/state.go` |
...
**`cmd/izen/runtime.go`** — `izen run "<prompt>"`: builds a **v1 runtime engine** via `pkg/runtime/wire.NewEngine` (`wire.go`), routes the prompt through `engine.Run(ctx, engine.Request{...})`, and prints the policy decision audit trail, plan, execution result, validation and metrics.
...
### 2.7 v1 runtime (`pkg/runtime`)

A self-contained state-machine runtime used by the CLI `izen run` (and integration tests):

- `pkg/runtime/engine/engine.go` — `Engine.Run` drives `Receive → Analyze → Plan → EvaluatePolicy → Execute → Validate → Done/Recover`; failures route to `attemptRecovery`.
- `pkg/runtime/engine/state.go` — explicit `transitionTable`.
- `pkg/runtime/wire/wire.go` — `NewEngine(cfg)` composes analyzer, planner, YAML policy evaluator, strategy/capability/validation registries, metrics collector, and the built-in strategies.
- `pkg/runtime/strategy/` — `DirectGenerationStrategy`, `IterativeStrategy` (ReAct tool loop), `DirectChatStrategy`, `Selector`, `ProviderRouter`, `Generator`, `ToolRunner`.
- `pkg/runtime/{analyzer,planner,policy,registry,metrics}` — facts, execution plans, declarative policy rules, capability/strategy/validation registries, metrics.
...
flowchart TB
    subgraph Boot["Process entry — cmd/izen"]
        ...
        RUN["cmd/izen/runtime.go (izen run)"] --> RWIRE["pkg/runtime/wire.NewEngine()"]
    end
...
### 4.5 v1 runtime result — `pkg/runtime/engine`

```
Result{ RunID, Request, State, Facts: *analyzer.Facts, Plan: *planner.Plan,
        Decision: *policy.Decision, Execution: *registry.Result,
        Validation: *registry.ValidationResult, Recovered, Err, Metrics[], Transitions[] }
State: idle → received → analyzed → planned → policy_evaluated → executing → validating → done | recovering → recovered | failed
```
...
2. `pkg/engine/ir` node states (`pending→ready→running→success/failed`) + `pkg/runtime/engine` run states (`received→…→done/recovered/failed`).
...
### 5.7 v0 mode engines and the v1 pipeline/CLI runtime are not wired together

- The layered pipeline engine is attached to the orchestrator and used for **model routing** (`routeModel`) and context/validation, but the mode engines (`/plan`, `/build`) execute their own independent LLM calls. The v1 `pkg/runtime` engine is **only** reachable via `izen run` and integration tests — a third, disconnected execution path.
- `/build` patch application runs through `internal/execution` + `internal/patch`, while the pipeline's Layer 4 validation DAG and `internal/review`'s sandbox verification are separate verification stacks that do not share results.
