...
> **Scope:** `cmd/izen`, `internal/**` (historical revisions also covered `pkg/**`, since migrated into `internal/`). This document reflects **only what exists in the codebase** — no idealized assumptions.
>
...
| **Layered Pipeline Engine (v1, "Layers 0–5")** | `internal/engine/v3/pipeline` + `internal/engine/v3/layer0..layer4` + `internal/engine/v3/telemetry` | Deterministic knowledge → capability → context → execution → validation pipeline with intent-based model routing | `internal/engine/v3/ir` Static/Dynamic IR + `internal/engine/v3/decision` |
| **Izen Agent Runtime V3 (CLI `izen run`)** | `internal/app/v3` + `internal/kernel` + `internal/events` + `internal/resource` + `internal/capability/v3` + `internal/retrieval/extractor` + `internal/ir` + `internal/op` + `internal/graph` + `internal/planner/v3` | Three-layer runtime: **Language Layer** (capability-constrained generation → evidence-based extraction → `ir.Artifact`) → **Planning Layer** (`GreenfieldPlanner`/`BrownfieldPlanner` → `op.Operation` → `graph.ExecutionGraph`) → **Runtime Layer** (`kernel.Engine` task lifecycle + `resource` adapters, side-effects on `internal/events`) | `internal/kernel` `ExecutionStatus` (pending → running → completed/failed/canceled) |
| **Microkernel intent compiler** | `internal/engine/v3/{intent,context,strategy,plan,ir/logical,lowerer,adapter,inference}` | Deterministic prompt → Logical IR → framework-specific FileArtifacts, zero LLM | — (pure transformations) |
...
- A second, engine-internal telemetry bus exists in `internal/engine/v3/telemetry` (`EventBus`, non-blocking, channel-backed) for the Layer 0–5 pipeline events (`layer0.knowledge_resolved`, `layer1.capability_detected`, `layer2.context_governed`, `layer3.pipeline_step`, `layer4.validation_dag`, `control.iteration`, `control.node_observed`, `control.terminated`).
...
**`cmd/izen/runtime.go`** — `izen run "<prompt>"`: routes the prompt through the **V3 pipeline** (`internal/app/v3`). It builds a pipeline via `app.NewPipeline(...)` (`internal/app/v3/pipeline.go`) with a `cliGenerator` adapting the configured `ai.Provider` to the `app.Generator` contract, subscribes `app.StatusLine` (`internal/app/v3/status.go`) to the pipeline's `internal/events` bus for terminal status output, and prints the V3 audit trail — capabilities, extraction attempts, artifact validations, planner metadata, and kernel task event counts.
...
### 2.5 Layered Pipeline Engine (Layers 0–5, `internal/engine/v3`)

**`internal/engine/v3/pipeline/engine.go`** — `Engine` (wired over a `lea.Engine` System of Record). `Run(ctx, Request)` executes the canonical six-step pipeline:

1. **Layer 0 — Knowledge resolution** (`internal/engine/v3/layer0/resolver.go`): `KnowledgeResolver.Resolve()` → `ResolvedKnowledge` (managers, lockfiles, file tree, AGENTS.md/CLAUDE.md/.cursorrules, project docs, README, active conventions, structural constraints, conflicts). Priority: RuntimeFacts(1) > AgentRules(2) > ProjectDocs(3) > HumanReads(4).
2. **Layer 1 — Capability detection** (`internal/engine/v3/layer1/detect.go`, `graph.go`): `layer1.Detect(root)` → `Graph` (stack + toolchain capability → command mapping). `CapabilityHeader(g)` (`internal/engine/v3/pipeline/header.go`) injects the stack into every LLM system prompt.
3. **Layer 2 — Governed context** (`internal/engine/v3/layer2/sor.go`, `governor.go`): `Sor` facade over the Lea graph → `ContextGovernor.Build(req, policy)` → immutable `ExecutionContext` (files, symbols, imports, token-budgeted stats).
4. **Layer 3 — Execution** (`internal/engine/v3/layer3/`): `PolicyGuard` routes each request to `RouteASTRewrite` or `RouteGenerative`; `Pipeline` runs `Understand → Plan → Execute → Review → Validate`; generative path uses a `Worker` (`routeWorker` in `internal/engine/v3/pipeline/engine.go`) that builds the prompt via `layer3.BuildPrompt` and parses patches via `layer3.LinePatchParser`; deterministic path uses `ASTRewriteHandler`. Stage transitions are published to the telemetry bus.
5. **Layer 4 — Validation DAG** (`internal/engine/v3/layer4/`): `DAG` executes `StageStructural` (SoR-based `StructuralValidator`) → `StageSyntax` (`SyntaxValidator`) always; `StageLint`/`StageBuild`/`StageTest` when `ValidationCapability` mode and the workspace capability graph exposes them.
6. **Layer 5 — Telemetry** (`internal/engine/v3/telemetry/`): every layer publishes to a non-blocking `EventBus`; `Timeline` records one request's ordered event stream; `StrategyOptimizer` folds pass rates back into policy recommendations.

The engine also exposes `RouteForMode(mode)` via the **intent-based model router** (`internal/engine/v3/pipeline/router.go`): `RouteProfile{Intent, Model, Provider, Policy, Reason}` with default per-intent Layer 2 budgets — `IntentReasoning` (24k tokens / 20 files / 1024 symbols), `IntentExecution` (16k / 16 / 512), `IntentInformational` (8k / 8 / 256).

### 2.6 Microkernel intent compiler (`internal/engine/v3` subpackages)

A pure, deterministic, zero-LLM compile pipeline for greenfield/framework generation:

```
intent.Classify (internal/engine/v3/intent/classify.go)
  → context.Collect (internal/engine/v3/context/collector.go)      [PlanningContext]
  → strategy.DetermineGoal (internal/engine/v3/strategy/*.go)      [Goal]
  → planner.Derive (internal/engine/v3/plan/planner.go)            [LogicalPlan of Steps]
  → normalizer/validator/policyEngine (internal/engine/v3/plan/*)  [Normalized → Validated]
  → IRPlanner (internal/engine/v3/planner/irplanner.go)            [Logical IR nodes]
  → lowerer.PlanLowerer (internal/engine/v3/lowerer/lowerer.go)    [Framework+Capability → MappedNode]
  → adapter.RenderNode (internal/engine/v3/adapter/*.go)           [FileArtifact]
```
...
- **Static/Dynamic IR**: `internal/engine/v3/ir/graph.go` (immutable `ExecutionGraph`/`ExecutionNode`/`Plan`) + `internal/engine/v3/ir/snapshot.go` (mutable `ExecutionSnapshot`, `NodeState`, `ObservationPayload`). `internal/engine/v3/decision/engine.go` (`StandardDecisionEngine`) is the *only* consumer of the Dynamic IR and emits `DirectiveContinue/Retry/RePlan/HumanApproval/Abort`.
- **Adapters** (`internal/engine/v3/adapter/`): pure renderers for `FrameworkNextJS`, `FrameworkAstro`, `FrameworkReactVite`, `FrameworkGoGin`, `FrameworkStaticWeb` (files `nextjs.go`, `astro.go`, `reactvite.go`, `gogin.go`, `staticweb.go`).

### 2.7 Izen Agent Runtime V3 (`internal/app/v3`, `internal/kernel`, `internal/events`, `internal/resource`, `internal/capability/v3`, `internal/retrieval/extractor`, `internal/op`, `internal/graph`, `internal/planner/v3`)

The **Agent Runtime V3** is the current, fully integrated runtime behind the CLI `izen run`. It is organized as a strict **three-layer pipeline** in which each layer is deliberately free of AI/LLM, I/O and prompt dependencies — the LLM is confined to the Language Layer's generation call, and all side effects are dispatched exclusively through the `internal/events` bus and the `internal/resource` adapters.

```
Language Layer                          Planning Layer                  Runtime Layer
─────────────                           ─────────────                  ─────────────
internal/capability/v3                          internal/planner/v3                    internal/kernel
...
internal/retrieval/extractor                            internal/op                        internal/resource
...
internal/ir                                  internal/graph                      internal/events
  ir.Artifact (path, kind, SHA-256)       ExecutionGraph (DAG,          MemoryEventBus
                                          InjectRepairOps)               task.* events
```

- **Language Layer — `internal/capability/v3` + `internal/retrieval/extractor` + `internal/ir`**: `capability.Registry` (`internal/capability/v3/registry.go`) stores and resolves `Capability` instances — each capability owns its own model-tier-aware `PromptRepresentation` and its `Validate(ctx, data) ValidationResult` (`internal/capability/v3/capability.go`), so the registry never interprets semantics. Built-in semantic capabilities (`internal/capability/v3/defaults.go`) — `portfolio_website`, `semantic_html`, `typescript`, `go_backend`, `generic_code` — constrain generation and gate artifacts. `internal/retrieval/extractor` turns raw LLM output into canonical `ir.Artifact` values via `MarkdownFenceExtractor` (` ```lang:path`, ` ```lang path`, `=== FILE: path`) and `JSONExtractor` (envelope schema); acceptance is a **pure function of `EvidenceFlag`s** (`EvValidFenceHeader`, `EvPathInHeader`, `EvFenceClosed`, `EvValidUTF8`, `EvValidJSONSchema`) — never a confidence score (`internal/retrieval/extractor/evidence.go`). `internal/ir` (`internal/ir/artifact.go`) is the dependency-free Artifact IR: `ArtifactKind` (file/symlink/meta), `NewArtifact` computes a deterministic SHA-256 `Hash`, and `ComputeHash` is the identity contract between extraction and validation.
- **Planning Layer — `internal/op` + `internal/graph` + `internal/planner/v3`**: `op.Operation` (`internal/op/op.go`) is the declarative Operation IR — a typed, target-resource-bound unit (`op.file.write`, `op.file.delete`, `op.exec.cmd`, `op.git.commit`) with `Preconditions` and `Timeout`, validated at construction; it never mutates a target directly. `graph.ExecutionGraph` (`internal/graph/graph.go`) is a thread-safe DAG of `OpNode`s run on the kernel engine, with runtime mutation via `InjectRepairOps` (inserts repair operations on a node failure and rewires downstream dependencies). `internal/planner/v3` exposes the `Planner` contract (`Plan(ctx, intent, artifacts) *PlanResult`); `GreenfieldPlanner` (`internal/planner/v3/greenfield`) deterministically lowers every file artifact into a parallel/topological batch of `OpWriteFile` writes (zero tool calls), while `BrownfieldPlanner` (`internal/planner/v3/brownfield`) builds an edit-and-verify graph (`bf-write` → `bf-verify` compile/test node) and runs the closed-loop `ExecuteAndRepair` — analyzing failure output into `FailureReport`/`FailureSymptom` and injecting repair ops until green or the repair budget is exhausted.
- **Runtime Layer — `internal/kernel` + `internal/events` + `internal/resource`**: `kernel.Engine` (`internal/kernel/engine.go`) executes `Executable` tasks with a task-scoped context, per-task `Timeout`, and deterministic terminal classification (`StatusCompleted`/`StatusFailed`/`StatusCanceled`) from cancellation cause / deadline / explicit runtime cancel (`internal/kernel/status.go`, `runtime.go`). `internal/events` is the dependency-free runtime event model (`task.started`, `task.completed`, `task.failed`, `task.canceled`, `budget.exceeded`, `state.checkpoint`); `MemoryEventBus` (`internal/events/bus.go`) is a thread-safe, non-blocking pub/sub — each subscription owns a buffered channel drained by its own goroutine, and `Publish` never blocks (slow observers drop events). `internal/resource` (`internal/resource/resource.go`) is the **Resource Target Inversion** abstraction: `Resource` exposes `ValidateState` / `Snapshot` / `Restore` over `file`, `git` and `terminal` adapters, so the graph and kernel interact with target state through one deterministic interface and never touch a file, process or repository directly.
- **`internal/app/v3`** (`internal/app/v3/pipeline.go`) is the integration seam that wires the three layers into a single callable `Pipeline` (`app.NewPipeline(...)`, options `WithRegistry`/`WithExtractors`/`WithGenerator`/`WithEventBus`/`WithRoot`/`WithMode`/`WithMaxAttempts`/`WithMaxRepairs`). `Run` routes a prompt through capability resolution → system prompt → generation → evidence-based extraction (rejected extractions re-prompt with the evidence payload) → validation gate (failed artifacts re-prompt with rejection reasons) → `plan()` (auto greenfield/brownfield detection; parent dirs created only after the gate) → `execute()` on `kernel.NewEngine(p.bus)`. `app.StatusLine` renders kernel events as compact status lines for terminal observers.
...
flowchart TB
    subgraph Boot["Process entry — cmd/izen"]
        ...
        RUN["cmd/izen/runtime.go (izen run)"] --> APPP["internal/app/v3: NewPipeline + Run"]
    end

    subgraph V3["Agent Runtime V3 — internal/* (Language → Planning → Runtime)"]
        CAP["internal/capability/v3 — Registry / semantics"]
        EXTR["internal/retrieval/extractor — EvidenceFlags → ir.Artifact"]
        PLAN3["internal/planner/v3 — GreenfieldPlanner / BrownfieldPlanner"]
        OP["internal/op — Operation IR"]
        GEX["internal/graph — ExecutionGraph / InjectRepairOps"]
        KERN["internal/kernel — Engine / task lifecycle"]
        RES["internal/resource — file / git / terminal adapters"]
        EVBUS3["internal/events — MemoryEventBus"]
        APPP --> CAP
        APPP --> EXTR
        APPP --> PLAN3
        PLAN3 --> OP
    ...
    subgraph MICRO["Microkernel intent compiler — internal/engine/v3"]
    ...
    subgraph PIPE["Layered Pipeline Engine — internal/engine/v3/pipeline"]
...
2. **Deterministic fast paths** (each completes in <1ms, zero LLM):
   ...
   - **Microkernel / Intent Compiler** (`MicrokernelPlanner.TryPlan`, `IntentCompilerPlanner.TryPlan`) run the `internal/engine/v3` pipeline (Step 2.6) for greenfield prompts → concrete `FileArtifact` targets, zero model tokens.
...
5. `planResultMsg{Tasks, IsFastTrack, Microkernel, IntentCompiler, TokenInput, TokenOutput}` → UI stages tasks, mirrors into `session.ContextLedger` (via `bridgePlanToLedger`), applies the **ScopeGuard** (`internal/boundary/scopeguard` `ValidateStagedPlan`), and awaits user approval (`planApprovalActions` / fast-track auto-approval).
...
sequenceDiagram
    ...
    participant MCP as internal/engine/v3 microkernel (intent→IR→lowerer)
    participant PIPE as internal/engine/v3/pipeline (Layers 0-5)
...
### 4.1 `InferenceSet` & evidence — `internal/engine/v3/inference`

Defined in `internal/engine/v3/inference/inference.go`, `facts.go`, `policy.go`, `evidence.go`.
...
### 4.2 IR nodes — Static & Dynamic (`internal/engine/v3/ir`)

**Static IR (`internal/engine/v3/ir/graph.go`):**
...
**Dynamic IR (`internal/engine/v3/ir/snapshot.go`, `internal/engine/v3/ir/state.go`):**
...
`SnapshotReader` is the only interface the `StandardDecisionEngine` (`internal/engine/v3/decision/engine.go`) consumes; it produces `Decision{Directive: continue|retry|replan|human_approval|abort}`.

**Logical IR (`internal/engine/v3/ir/logical`):**
...
**Transformation chain** (from `internal/engine/v3/plan/step.go` package doc):

```
Goal (internal/engine/v3/strategy) → LogicalPlan → NormalizedPlan → ValidatedPlan → ExecutablePlan
```

Step kinds: `create | modify | delete | read | run | verify`.

### 4.3 `ExecutionContext` (Layer 2) — `internal/engine/v3/layer2`

`internal/engine/v3/layer2/governor.go` + `sor.go`:
...
Produced by `ContextGovernor.Build` from the `Sor` (Lea System of Record); consumed by `layer3.BuildPrompt` and the pipeline `Result.Context`. Per-intent budgets come from `internal/engine/v3/pipeline/router.go` `DefaultPolicies`.

### 4.4 File artifacts & patches

**Adapter output (`internal/engine/v3/adapter/adapter.go`):**
...
**Layer 3 patch (`internal/engine/v3/layer3/ast_rewrite.go` + `worker.go`):**
...
**Kernel task lifecycle (`internal/kernel/status.go`, `task.go`, `runtime.go`):**
...
The `kernel.Engine` (`internal/kernel/engine.go`) derives a task-scoped context (applying `Timeout` via `context.WithTimeoutCause` with `ErrTaskTimeout`), classifies the terminal status from cancellation cause / deadline / explicit runtime cancel, and emits `task.started` → `task.completed`/`task.failed`/`task.canceled` events.

**Operation IR (`internal/op/op.go`):**
...
**Planner output (`internal/planner/v3/planner.go`):**
...
**Event model (`internal/events/event.go`):**
...
- The **microkernel / intent-compiler** path (`internal/engine/v3`, `internal/modes/plan/microkernel.go`, `intentcompiler.go`) produces `[]Task` from `ExecutablePlan`/`FileArtifact`s **with zero LLM**, while `internal/modes/plan/engine.go` `ProcessFromLedger` runs an independent LLM JSON-synthesis engine with its own `Task` schema, parser (`ParseJSONPlan`, `ParseMarkdownToTasks`), and filter chain.
...
- The `internal/engine/v3/plan` microkernel plans (`create/modify/read/run/verify` Steps) and the mode-level `Task` types are **different abstractions over the same work** with no shared interface.

### 5.2 Two independent event/telemetry buses

- `internal/events.Bus` (domain events, UI projection) and `internal/engine/v3/telemetry.EventBus` (layer/control telemetry) are **separate implementations with overlapping purpose** (non-blocking channel pub/sub, drop-on-full, per-subscription goroutines). The UI bridges the telemetry bus via `controlFactMsg` and the domain bus via `domainEventMsg`; the pipeline engine can emit to either depending on wiring.
...
2. `internal/kernel` task statuses (`pending→running→completed/failed/canceled`) + `internal/engine/v3/ir` node states (`pending→ready→running→success/failed`).
...
- Mode ↔ intent mapping is done multiple times: `modes.Mode` (internal/modes), `pipeline.Intent` (`internal/engine/v3/pipeline/intent.go` `IntentForMode`), `router` classification strings, `classifier.FailureClass`, and `events.FailureClassification` (transient/recoverable/permanent) are **four separate taxonomies** that must be kept in sync by convention.
...
`internal/prompt` (canonical contracts), `internal/gateway` (compressor + casual-chat), `internal/engine/v3/layer3` (`BuildPrompt`, `CapabilityHeader`), and `internal/engine/v3/pipeline` (`header.go`) all assemble prompt content. The workspace-capability header is injected through a package-level `prompt.SetWorkspaceCapabilities` global (`program.go:134`) — a hidden global dependency.

### 5.7 v0 mode engines and the V3 pipeline are not wired together

- The V3 Agent Runtime (`internal/app/v3`) is **only** reachable via `izen run` and integration tests — a self-contained execution path independent of the mode engines and the layered pipeline. The TUI `/plan`, `/build`, `/investigate`, `/review` mode engines execute their own independent LLM calls and their own patch/verify stacks.
...
The rich microkernel pipeline (`inference` + `IRPlanner` + `lowerer` + `adapter`) currently supports **5 frameworks** (nextjs, astro, react-vite, go-gin, static-web) and a single greenfield-web node set (pages/sections/components/endpoints/migrations/styles/scripts). The `internal/discovery/recon` archetype detector and `internal/project` detection are separate detection stacks from `layer1` capability detection — three overlapping "what is this project" mechanisms.

---

## Historical Context / V2 Purge Note

The **V2 legacy runtime** (`internal/runtime/v3/` — the `wire.NewEngine` state-machine runtime with `analyzer`, `engine`, `metrics`, `planner`, `policy`, `registry`, `strategy`, and `wire` subpackages) was **permanently deleted** from the repository. It was superseded by the **Izen Agent Runtime V3** described throughout this document (`internal/app/v3`, `internal/kernel`, `internal/events`, `internal/resource`, `internal/capability/v3`, `internal/retrieval/extractor`, `internal/ir`, `internal/op`, `internal/graph`, `internal/planner/v3`).

The V2 runtime was a self-contained, state-driven orchestration path (`Receive → Analyze → Plan → EvaluatePolicy → Execute → Validate → Done/Recover`) reachable only via `izen run` and integration tests — a disconnected execution path alongside the mode engines and the layered pipeline. The CLI `izen run` now routes strictly through the V3 `internal/app/v3` pipeline. This section is retained solely as historical record; no V2 code or references remain in the repository.
