...
func TestDecideRetryWithinBudget(t *testing.T) {
	plan := pipelinePlan(t)
	eng := NewStandardDecisionEngine(WithRetryPolicy(RetryPolicy{
		MaxAttempts: 2,
...
func TestDecideRetryExhaustedSkipsToCriticalHandling(t *testing.T) {
	plan := pipelinePlan(t)
	eng := NewStandardDecisionEngine(WithRetryPolicy(RetryPolicy{MaxAttempts: 2}))
	snap := ir.NewExecutionSnapshot(plan)
	failNode(t, snap, "a", 2)
...
func TestDecideFileCheckNeverRetried(t *testing.T) {
	plan := pipelinePlan(t)
	eng := NewStandardDecisionEngine(WithRetryPolicy(RetryPolicy{MaxAttempts: 5}))
	snap := ir.NewExecutionSnapshot(plan)
	succeedNode(snap, "a")
...
func TestDecideRePlanOnGraphInvalidation(t *testing.T) {
	plan := pipelinePlan(t)
	eng := NewStandardDecisionEngine(WithRetryPolicy(RetryPolicy{MaxAttempts: 1}))
	snap := ir.NewExecutionSnapshot(plan)
...
func TestRetryPolicyBounds(t *testing.T) {
	p := RetryPolicy{MaxAttempts: 3, Backoff: func(a int) time.Duration { return time.Duration(a) * time.Second }}
