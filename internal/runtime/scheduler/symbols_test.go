...
func TestEvidence_RedundantSymbolRejection(t *testing.T) {
	...
	if err := os.WriteFile(sink.path, []byte(baseline), 0600); err != nil {
		t.Fatal(err)
	}
	...
	worker := func(_ context.Context, _ ExecutionStep, slice ContextSlice, buffer *executor.ProposalStagingBuffer) (StreamResult, error) {
		for _, name := range []string{"AVAILABLE SYMBOLS", "widget", "Exported", "existingHelper", "limit"} {
			if !strings.Contains(slice.RenderPrompt(), name) {
				t.Fatalf("missing scope symbol %s", name)
			}
		}
		staged = buffer
	...
	if err != nil || string(actual) != baseline {
		t.Fatalf("workspace changed: %q %v", actual, err)
	}
	if len(state.RecoveryContext.ReuseSymbols) != 1 || state.RecoveryContext.ReuseSymbols[0] != "existingHelper" {
		t.Fatalf("missing reuse guidance: %+v", state.RecoveryContext)
	}
	continuation := func(_ context.Context, _ ExecutionStep, slice ContextSlice, buffer *executor.ProposalStagingBuffer) (StreamResult, error) {
		if !strings.Contains(slice.RecoveryInstructions, "existingHelper") || !strings.Contains(slice.RecoveryInstructions, "REDUNDANT_SYMBOL") {
			t.Fatalf("missing actionable continuation: %s", slice.RenderPrompt())
		}
	...
	if err != nil || second.Outcome != StepOutcomeComplete || second.Patches != 1 || sink.calls != 1 {
		t.Fatalf("existing helper modification rejected: %+v %v", second, err)
	}
	if state.RecoveryContext.Reason != "" {
		t.Fatalf("recovery not cleared after commit: %+v", state.RecoveryContext)
	}
}
...
func TestEvidence_PublicDeclarationAllowed(t *testing.T) {
	...
	if err != nil || result.Outcome != StepOutcomeComplete || sink.calls != 1 {
		t.Fatalf("public API rejected: %+v %v", result, err)
	}
}
