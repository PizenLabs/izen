...
func TestPipelineGreenfieldWritesValidatedArtifacts(t *testing.T) {
	...
	waitForEvent(t, evCh, events.EventTaskStarted, "")
	waitForEvent(t, evCh, events.EventTaskCompleted, "")
}
...
func collectBusEvents(t *testing.T, bus *events.Bus) <-chan events.DomainEvent {
	t.Helper()
	ch := make(chan events.DomainEvent, 64)
	bus.SubscribeAll(func(ev events.DomainEvent) { ch <- ev })
	return ch
}

// eventTaskID extracts the task identity from a task lifecycle event
// payload, or "" when the event carries none.
func eventTaskID(ev events.DomainEvent) string {
...
// id) arrives.
func waitForEvent(t *testing.T, ch <-chan events.DomainEvent, typ string, taskID string) {
	t.Helper()
	deadline := time.Now().Add(3 * time.Second)
	for time.Now().Before(deadline) {
		select {
		case ev := <-ch:
			if ev.Type() == typ && (taskID == "" || eventTaskID(ev) == taskID) {
				return
			}
