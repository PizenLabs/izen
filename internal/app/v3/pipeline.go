...
//
// Side-effects are dispatched exclusively through the internal/events bus so
// a TUI (or the CLI's own terminal observer) can render real-time status
// updates.
package app
...
import (
	...
	"github.com/PizenLabs/izen/internal/capability/v3"
	...
	"github.com/PizenLabs/izen/internal/planner/v3"
...
func NewPipeline(opts ...Option) (*Pipeline, error) {
	...
	p.bus.SubscribeAll(func(ev events.DomainEvent) { p.recordEvent(ev) })
	return p, nil
}
...
func (p *Pipeline) clarifyGate(ctx context.Context, intent *ir.IntentIR) error {
	...
	p.bus.Publish(events.NewClarificationRequired("pipeline", questions))
...
func (p *Pipeline) dispatchClarification(ctx context.Context, questions []ir.ClarificationQuestion, resp chan<- ir.ClarificationResponse) {
	...
	if err := p.clarifier.Clarify(ctx, questions, resp); err != nil {
		...
		p.bus.Publish(events.NewTaskFailed("pipeline", err))
		resp <- ir.ClarificationResponse{Answers: ir.DefaultAnswers(questions)}
	}
}
...
func (p *Pipeline) emitStage(ctx context.Context, stage string) {
	...
	p.bus.Publish(events.NewStateCheckpoint("pipeline", StageEvent{Stage: stage}))
}

// recordEvent appends an observed event to the best-effort audit log.
func (p *Pipeline) recordEvent(ev events.DomainEvent) {
	p.eventsMu.Lock()
...
func (p *Pipeline) snapshotEvents() []events.DomainEvent {
	p.eventsMu.Lock()
	defer p.eventsMu.Unlock()
