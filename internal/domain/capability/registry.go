...
// stores CapabilityContract instances keyed by CapabilityID and resolves requested id
...
type Registry struct {
	mu   sync.RWMutex
	caps map[CapabilityID]CapabilityContract
	ids  []CapabilityID
}

// NewRegistry returns an empty capability registry.
func NewRegistry() *Registry {
	return &Registry{caps: make(map[CapabilityID]CapabilityContract)}
}
...
func (r *Registry) Register(c CapabilityContract) error {
	if c == nil {
...
func (r *Registry) RegisterAll(caps ...CapabilityContract) error {
	for _, c := range caps {
...
func (r *Registry) Lookup(id CapabilityID) (CapabilityContract, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	c, ok := r.caps[id]
	return c, ok
...
// Resolve resolves the requested id set into the active CapabilityContract instances,
...
func (r *Registry) Resolve(ids ...CapabilityID) ([]CapabilityContract, error) {
	out := make([]CapabilityContract, 0, len(ids))
	r.mu.RLock()
	defer r.mu.RUnlock()
	for _, id := range ids {
		c, ok := r.caps[id]
...
func (r *Registry) ResolveOne(id CapabilityID) (CapabilityContract, error) {
