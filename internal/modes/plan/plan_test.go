...
func TestParseJSONPlan_SanitizeJSONContent(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{"markdown fences with json tag", "```json\n{\"a\":1}\n```", "{\"a\":1}"},
		{"markdown fences", "```\n{\"a\":1}\n```", "{\"a\":1}"},
		{"bare JSON", "{\"a\":1}", "{\"a\":1}"},
		{"leading // comment", "// this is a comment\n{\"a\":1}", "{\"a\":1}"},
		{"leading // comments", "// plan output\n// generated by qwen\n{\"a\":1}", "{\"a\":1}"},
		{"trailing // comment", "{\"a\":1} // end", "{\"a\":1}"},
		{"leading /* block */", "/* comment */\n{\"a\":1}", "{\"a\":1}"},
		{"multiline /* block */", "/* line 1\n   line 2 */\n{\"a\":1}", "{\"a\":1}"},
		{"inline // in strings", "{\"a\":\"//not a comment\"}", "{\"a\":\"//not a comment\"}"},
		{"non-JSON text before", "some text here\n{\"a\":1}", "{\"a\":1}"},
		{"non-JSON before and after", "intro\n{\"a\":1}\ntrailer", "{\"a\":1}"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := sanitizeJSONContent(tt.input)
			if got != tt.want {
				t.Errorf("sanitizeJSONContent(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}
