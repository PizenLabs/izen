...
// extractRawIntentFromAskOutput parses the structured /ask output for the
// raw_intent field. Returns the raw_intent value if found, or the full content
// as fallback.
func extractRawIntentFromAskOutput(content string) string {
	for _, line := range strings.Split(content, "\n") {
		trimmed := strings.TrimSpace(line)
		lowerLine := strings.ToLower(trimmed)
		if strings.HasPrefix(lowerLine, "raw_intent:") || strings.HasPrefix(lowerLine, "raw intent:") {
	...
	// Fallback: return the full content (pre-reform compatibility).
	return content
}

// bridgeAskHandoffToLedger persists the structured raw-intent handoff pack
// from /ask mode into the canonical session.ContextLedger.
//
// REFORM: The raw_intent field (user's original request) is stored separately
// in UserRawIntent. The full assistant output (including domain classification
// and scope tags) goes into AssistantDiscussionNotes. This separation ensures
// that /build receives ONLY the raw intent + /plan artifacts, never pre-baked
// structural code steps.
func (m *model) bridgeAskHandoffToLedger(content string) {
	...
	// Extract raw_intent and store separately from discussion notes.
	rawIntent := extractRawIntentFromAskOutput(content)
	if rawIntent != "" {
		ledger.UserRawIntent = rawIntent
	}
	// Full assistant output (domain, tags, discussion) goes into notes.
	ledger.AssistantDiscussionNotes = content
...
func (m *model) primeHandoffFromLedger(mode modes.Mode) {
	...
	if l.UserRawIntent != "" && !strings.Contains(m.handoffLedgerContent, strings.TrimSpace(l.UserRawIntent)) {
		if m.handoffLedgerContent != "" {
			...
			m.handoffLedgerContent = "### USER RAW INTENT\n" + l.UserRawIntent + "\n"
		}
	}
